...
<body>
<form action="queryf.php" method="GET">
Mot en français à rechercher: <input type="text" name="w"><br>
<input type="submit"><small style="margin-left:50px;"><a href="query.php">bamanankan na?</a></small>
</form>
...
$pattern="/[^abcdefghijklmnopqrstuvwxyzéèçàùâêîôûäëïöü\s!\-]/i";
$rejeter=preg_match($pattern,$word);

if ($rejeter) { echo "<br>\n$word : Il y a une ou des lettres qui ne sont pas dans l'alphabet du français<br>";$nerr+=1;}
...
$sql = "SELECT * FROM lx WHERE (lx.gf REGEXP '([[:blank:][:punct:]]|^)$word([[:blank:][:punct:]]|$)' ) ORDER BY lx.gf ";
#
...
if ($retour === FALSE) {
	...
	if (!$enreg = mysqli_fetch_array($retour1, MYSQLI_BOTH)) {  # not found in gf
		echo "$word absent des gloses principales, recherche dans les gloses secondaires<br />\n";
		$sql_gvf="SELECT id FROM gvf WHERE gvf.gvf REGEXP '([[:blank:][:punct:]]|^)$word([[:blank:][:punct:]]|$)' ORDER BY gvf.gvf";
		$retgvf = mysqli_query($mydb,$sql_gvf);
		if ($retgvf != FALSE) {
			$gvf_idlist="";
			while ($enreg_gvf = mysqli_fetch_array($retgvf, MYSQLI_BOTH)) {
    				$ngvf+=1;
    				$gvf_idlist=$gvf_idlist.$enreg_gvf["id"].", ";
    			}
    			if ($gvf_idlist!="") {
    				$gvf_idlist=chop($gvf_idlist,", ");
    				$sql = "SELECT * FROM lx WHERE lx.id IN ($gvf_idlist)";
    				$retour = mysqli_query($mydb,$sql);
				if ($retour === FALSE) {
    				...
    				echo "$word absent des gloses secondaires, recherche dans les exemples<br>\n";
    				$sql_lxx="SELECT id FROM lxx WHERE lxx.exf REGEXP '([[:blank:][:punct:]]|^)$word([[:blank:][:punct:]]|$)'";
				$retlxx = mysqli_query($mydb,$sql_lxx);
				if ($retlxx != FALSE) {
					$lxx_idlist="";
					while ($enreg_lxx = mysqli_fetch_array($retlxx, MYSQLI_BOTH)) {
		    				$nlxx+=1;
		    				$lxx_idlist=$lxx_idlist.$enreg_lxx["id"].", ";
		    			}
		    			if ($lxx_idlist!="") {
		    				$lxx_idlist=chop($lxx_idlist,", ");
		    				$sql = "SELECT * FROM lx WHERE lx.id IN ($lxx_idlist)";
		    				$retour = mysqli_query($mydb,$sql);
						if ($retour === FALSE) {
							echo "<br/>La requête a échoué:$sql\n";
						}
		    				...
		    				echo "$word absent des exemples<br>\n";
		    			}
		    		}
    			}
		}
